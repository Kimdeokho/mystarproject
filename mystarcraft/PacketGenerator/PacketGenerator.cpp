...
{
	//"argv에 프로젝트속성 -> 디버그 -> 명령줄 인수" 에다가 읽을 파일주소를 넣었다.
	...
	_sntprintf_s(StructureFileName, sizeof(StructureFileName), _T("%s_Structure.h"), argv[2]);
	_sntprintf_s(ReadPacketFileName, sizeof(ReadPacketFileName), _T("%s_ReadPacket.h"), argv[2]);
	_sntprintf_s(WritePacketFileName, sizeof(WritePacketFileName), _T("%s_WritePacket.h"), argv[2]);

	ZeroMemory(Protocol, sizeof(PROTOCOL) * MAX_PROTOCOL_COUNT);

	FILE *SourceFile = NULL;
	_tfopen_s(&SourceFile, argv[1], _T("r"));
	...
	while (_fgetts(Data, MAX_LINE_LENGTH, SourceFile))
	{
		...
		// 라인 안에 ,가 있으면 프로토콜입니다.
		TCHAR *Pos = _tcsstr(TempData, _T(",")); //문자열을 찾는다, 제일먼저 발견된 문자열의 주소를 반환한다.
		if (Pos)
		{
			if (!_tcsstr(TempData, _T("VERSION")))
				_tcsncpy_s(Protocol[ProtocolPos++].Name
				,TempData, Pos - TempData/*문자길이*/);
			else
			{
		...
		else
		{
			...
			if (_tcslen(TempData) != 0)
			{
				if (IsParameterOpen)
				{
					...
					if (Pos)
					{
						_tcsncpy_s(Protocol[ProtocolPos-1].Parameters[ParameterPos++].Type, 
							 TempData, Pos - TempData/*문자길이*/);
					}
					else
						continue;
					...
					if (StartPos)
					{
						...
						_tcsncpy_s(Length,  StartPos + 1, EndPos - StartPos - 1);
						Protocol[ProtocolPos-1].Parameters[ParameterPos-1].Length = _tstoi(Length);

						_tcsncpy_s(Protocol[ProtocolPos-1].Parameters[ParameterPos-1].Name, 
					...
					else
					{
						...
						_tcscpy_s(Protocol[ProtocolPos-1].Parameters[ParameterPos-1].Name, Pos + 1);
					}
				}
			}
		}
	...
	_tfopen_s(&StructureFile, StructureFileName, _T("w"));
	...
	// Read 함수 만들기
	FILE *ReadPacketFile = NULL;
	_tfopen_s(&ReadPacketFile, ReadPacketFileName, _T("w"));
	...
	// Write 함수 만들기
	FILE *WritePacketFile = NULL;
	_tfopen_s(&WritePacketFile, WritePacketFileName, _T("w"));
	...
	for (DWORD i=0;i<ProtocolPos;i++)
	{
		...
		for (DWORD j=0;j<Protocol[i].ParametersCount;j++)
		{
			_tcslwr_s(Protocol[i].Parameters[j].Name);

			if (Protocol[i].Parameters[j].Length > 0)
				_ftprintf(WritePacketFile, _T(",const %s *%s"), Protocol[i].Parameters[j].Type , Protocol[i].Parameters[j].Name);
			else
				_ftprintf(WritePacketFile, _T(", %s %s"), Protocol[i].Parameters[j].Type, Protocol[i].Parameters[j].Name);
		}
		...
		for (DWORD j=0;j<Protocol[i].ParametersCount;j++)
		{
			...
			else if (!_tcscmp(Protocol[i].Parameters[j].Type, _T("WCHAR")))
			{
				...
				{					
					...
					//_ftprintf(WritePacketFile, _T("\twcsncpy(_%s, %s, %d);\n"), Protocol[i].Parameters[j].Name , Protocol[i].Parameters[j].Name , Protocol[i].Parameters[j].Length);
					_ftprintf(WritePacketFile, _T("\twcsncpy_s(_%s , %d , %s, _TRUNCATE);\n"), Protocol[i].Parameters[j].Name ,Protocol[i].Parameters[j].Length, Protocol[i].Parameters[j].Name);
