...
VOID CProfile::Begin(LPTSTR name)
{
	...
	_tcscpy_s(mSamples[i].Name, name);
...
VOID CProfile::DumpProfileOutput(VOID)
{
	...
	while (i < MAX_PROFILE_SAMPLES && mSamples[i].Valid == TRUE)
	{
		...
		_sntprintf_s(Aveage,	_countof(Aveage), _T("%4.2f"),	AveageTime);
		_sntprintf_s(Minimum,	_countof(Minimum), _T("%4.2f"),	MinimumTime);
		_sntprintf_s(Maximum,	_countof(Maximum), _T("%4.2f"),	MaximumTime);
		_sntprintf_s(Number,	_countof(Number), _T("%7d"),		mSamples[i].ProfileInstance);

		_tcsncpy_s(IndentedName, sizeof(IndentedName), mSamples[i].Name, 256);

		for (Indent=0;Indent<mSamples[i].NumberOfParents;Indent++)
		{
			_sntprintf_s(Name, _countof(Name), _T("  %s"), IndentedName);
			_tcsncpy_s(IndentedName,sizeof(IndentedName), Name, 256);
		}

		_sntprintf_s(Line, _countof(Number), _T("%7s : %7s : %7s : %7s : %s\n"), Aveage, Minimum, Maximum, Number, IndentedName);
		_tprintf(Line);
		i++;
	}
...
VOID CProfile::storeProfileInHistory(LPTSTR name, double percent)
{
	...
	if (i < MAX_PROFILE_SAMPLES)
	{
		_tcsncpy_s(mHistories[i].Name, sizeof(mHistories[i].Name) , name, MAX_PATH);
