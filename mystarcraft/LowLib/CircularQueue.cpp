...
BYTE *CCircularQueue::Push(VOID *object, BYTE *data, DWORD dataLength, LPCSTR remoteAddress, USHORT remotePort)
{
	...
	strncpy_s(mQueue[TempTail].RemoteAddress, _countof(mQueue[TempTail].RemoteAddress), remoteAddress, _TRUNCATE);
...
BOOL CCircularQueue::Pop(VOID **object, BYTE *data, DWORD &dataLength, LPSTR remoteAddress, USHORT &remotePort)
{
	...
	*object		= mQueue[TempHead].Object;
	...
	strncpy_s(remoteAddress, _countof(mQueue[TempHead].RemoteAddress), mQueue[TempHead].RemoteAddress, _countof(mQueue[TempHead].RemoteAddress) - 1 );
