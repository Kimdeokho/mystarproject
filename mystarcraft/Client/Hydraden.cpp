...
void CHydraden::Initialize(void)
{
	...
	m_com_anim = new CCom_ZBuildingAnim(L"Z_HYDRADEN" , m_matWorld);
...
void CHydraden::Update(void)
{
	...
	else if(BUILD == m_unitinfo.state)
	{
		...
		if(m_build_hp >= m_build_maxhp)
		{
			...
			CIngame_UIMgr::GetInstance()->BuildTech_Update(Z_HYDRADEN , 1 , m_eteamnumber);
		}
	}
...
void CHydraden::Inputkey_reaction(const int& nkey)
{
	if('Q' == nkey)
	{
		if( false == m_upg_info[UPG_Z_BD0].proceeding &&
			m_upg_info[UPG_Z_BD0].upg_cnt < 1)
		{
			m_upg_info[UPG_Z_BD0].proceeding = true;
			m_upg_info[UPG_Z_BD0].obj_num = m_obj_id;
	...
	if('W' == nkey)
	{
		if( false == m_upg_info[UPG_Z_BD1].proceeding &&
			m_upg_info[UPG_Z_BD1].upg_cnt < 1)
		{
			m_upg_info[UPG_Z_BD1].proceeding = true;
			m_upg_info[UPG_Z_BD1].obj_num = m_obj_id;
			m_unitinfo.state = DEVELOPING;
		}
	}
	if('A' == nkey)
	{
		if( false == m_upg_info[UPG_Z_BD3].proceeding &&
			m_upg_info[UPG_Z_BD3].upg_cnt < 1)
		{
			m_upg_info[UPG_Z_BD3].proceeding = true;
			m_upg_info[UPG_Z_BD3].obj_num = m_obj_id;
			m_unitinfo.state = DEVELOPING;
		}
	}
}

void CHydraden::Inputkey_reaction(const int& firstkey , const int& secondkey)
{

}
bool CHydraden::Input_cmd(const int& nkey , bool* waitkey)
{
	if('Q' == nkey)
		return true;
	if('W' == nkey)
...
void CHydraden::Update_Cmdbtn(void)
{
	CUI_Cmd_info* pcmd = CIngame_UIMgr::GetInstance()->GetCmd_info();

	if(IDLE == m_unitinfo.state )
	{
		if( !m_upg_info[UPG_Z_BD0].proceeding && m_upg_info[UPG_Z_BD0].upg_cnt == 0)
		pcmd->Create_Cmdbtn(0 , L"BTN_Z_BD0" , BTN_Z_BD0 , true , L"Q");
		if( !m_upg_info[UPG_Z_BD1].proceeding && m_upg_info[UPG_Z_BD1].upg_cnt == 0)
		pcmd->Create_Cmdbtn(1 , L"BTN_Z_BD1" , BTN_Z_BD1 , true , L"W");

		if( !m_upg_info[UPG_Z_BD3].proceeding && m_upg_info[UPG_Z_BD3].upg_cnt == 0)
		{
			if(0 < CIngame_UIMgr::GetInstance()->Get_BuildTech(Z_LAIR ,m_eteamnumber))
				pcmd->Create_Cmdbtn(3 , L"BTN_Z_BD3" , BTN_Z_BD3 , true , L"A");
			else
				pcmd->Create_Cmdbtn(3 , L"BTN_Z_BD3" , BTN_Z_BD3 , false , L"A");
		}
	}
	else if(DEVELOPING == m_unitinfo.state)
		pcmd->Create_Cmdbtn(8 , L"BTN_CANCLE" , BTN_CANCLE , true);
}
void CHydraden::Update_Wireframe(void)
{
	...
	{		
		...
		pui = new CUI_Wireframe(L"WIRE_HYDRADEN" , D3DXVECTOR2(interface_pos.x + 165, interface_pos.y + 390 ));
		CFontMgr::GetInstance()->SetInfomation_font(L"Zerg Hydraden" ,interface_pos.x + 320 , interface_pos.y + 390 );
		...
		else if(DEVELOPING == m_unitinfo.state)
		{
			...
			if(true == m_upg_info[UPG_Z_BD0].proceeding && 
				m_upg_info[UPG_Z_BD0].obj_num == m_obj_id)
			{
				pui = new CUI_form(L"BTN_Z_BD0" , D3DXVECTOR2(interface_pos.x + 258 , interface_pos.x + 410));
				CIngame_UIMgr::GetInstance()->add_wireframe_ui(pui);
			}
			else if(true == m_upg_info[UPG_Z_BD1].proceeding && 
				m_upg_info[UPG_Z_BD1].obj_num == m_obj_id)
			{
				pui = new CUI_form(L"BTN_Z_BD1" , D3DXVECTOR2(interface_pos.x + 258 , interface_pos.x + 410));
				CIngame_UIMgr::GetInstance()->add_wireframe_ui(pui);
			}
			else if(true == m_upg_info[UPG_Z_BD3].proceeding && 
				m_upg_info[UPG_Z_BD3].obj_num == m_obj_id)
			{
				pui = new CUI_form(L"BTN_Z_BD3" , D3DXVECTOR2(interface_pos.x + 258 , interface_pos.x + 410));
				CIngame_UIMgr::GetInstance()->add_wireframe_ui(pui);
			}
		}
	}
