...
void CBrude::Initialize(void)
{
	...
	m_com_anim = new CCom_BrudeAnim(m_matWorld);
...
void CBrude::Inputkey_reaction(const int& nkey)
{
	if( false == m_unitinfo.is_active )
		return;
...
void CBrude::Inputkey_reaction(const int& firstkey , const int& secondkey)
{
	if( false == m_unitinfo.is_active )
		return;
...
bool CBrude::Input_cmd(const int& firstkey , const int& secondkey)
{
	return false;
}

void CBrude::Update_Cmdbtn(void)
{
...
void CBrude::Update_Wireframe(void)
{
	...
	{
		CUI* pui = NULL;
		pui = new CUI_Wireframe(L"WIRE_BROODLING" , D3DXVECTOR2(interface_pos.x + 165, interface_pos.y + 390 ));
		pui->Initialize();
		CIngame_UIMgr::GetInstance()->add_wireframe_ui(pui);

		CFontMgr::GetInstance()->SetInfomation_font(L"Zerg Broodling" ,interface_pos.x + 320 , interface_pos.y + 390 );
	}
...
void CBrude::SetDamage(const int& idamage , DAMAGE_TYPE edamagetype)
{
