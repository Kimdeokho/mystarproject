...
void CFactory::Update_Cmdbtn(void)
{
	if(IDLE == m_unitinfo.estate)
	{
		CComanderMgr::GetInstance()->Create_Cmdbtn(0 , L"BTN_VULTURE" , BTN_VULTURE , true);

		if(0 < CComanderMgr::GetInstance()->Get_T_BuildTech(T_FAC_ADDON))
		{
			CComanderMgr::GetInstance()->Create_Cmdbtn(1 , L"BTN_SIEGETANK" , BTN_SIEGETANK , true);
		}
		else
		{
			CComanderMgr::GetInstance()->Create_Cmdbtn(1 , L"BTN_SIEGETANK" , BTN_SIEGETANK , false);
		}

		if(0 < CComanderMgr::GetInstance()->Get_T_BuildTech(T_ARMOURY))
			CComanderMgr::GetInstance()->Create_Cmdbtn(2 , L"BTN_GOLIATH" , BTN_GOLIATH , true);
		else
			CComanderMgr::GetInstance()->Create_Cmdbtn(2 , L"BTN_GOLIATH" , BTN_GOLIATH, false);

		if( NULL == m_partbuilding )
			CComanderMgr::GetInstance()->Create_Cmdbtn(6 , L"BTN_FAC_ADDON" , BTN_FAC_ADDON , true);
