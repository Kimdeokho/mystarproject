			...
			true == ((CBuilding_Preview*)subpreview)->Install_check(m_subpreview_info , this))
		{
			...
			if(m_vgroundpos.y <= m_vPos.y)
			{
				...
				if(true == m_is_partinstall)
				{
					...
					if(T_FAC_ADDON == m_preview_info.ebuild)
					{
...
void CFactory::Inputkey_reaction(const int& nkey)
{
	...
	if('C' == nkey)
	{
		...
		CUI* preview = CComanderMgr::GetInstance()->GetPreview();
		CUI* subpreview = CComanderMgr::GetInstance()->GetSubPreview();
		((CBuilding_Preview*)preview)->SetPreviewInfo(L"T_FACADDON", T_FAC_ADDON , 2 , 2 , this);
		((CBuilding_Preview*)subpreview)->SetPreviewInfo(L"T_FACTORY", T_FACTORY , 3 , 4 , this);
	}
	if('L' == nkey)
	{
