...
void COverload::Initialize(void)
{
	...
	m_com_anim = new CCom_OverAnim(m_matWorld);
...
void COverload::Inputkey_reaction(const int& nkey)
{
...
void COverload::Inputkey_reaction(const int& firstkey , const int& secondkey)
{
...
bool COverload::Input_cmd(const int& nkey , bool* waitkey)
{
	if('U' == nkey)
	{
		waitkey[nkey] = true;
...
bool COverload::Input_cmd(const int& firstkey , const int& secondkey)
{
	return false;
}

void COverload::Update_Cmdbtn(void)
{
...
void COverload::Update_Wireframe(void)
{
...
void COverload::SetDamage(const int& idamage , DAMAGE_TYPE edamagetype)
{
