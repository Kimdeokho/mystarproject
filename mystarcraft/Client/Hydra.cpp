...
void CHydra::Initialize(void)
{
	CObj::unit_area_Initialize();
	CUnit::Initialize();

	m_eOBJ_NAME = OBJ_HYDRA;
	...
	m_energybar_ui = new CUI_Energy_bar(this , 25 , 30);
	m_energybar_ui->Initialize();
}

void CHydra::Update(void)
{
	CObj::area_update();
...
void CHydra::Inputkey_reaction(const int& nkey)
{
	if( false == m_unitinfo.is_active )
		return;
...
void CHydra::Inputkey_reaction(const int& firstkey , const int& secondkey)
{
	if( false == m_unitinfo.is_active )
		return;
...
bool CHydra::Input_cmd(const int& firstkey , const int& secondkey)
{
	return false;
}

void CHydra::Update_Cmdbtn(void)
{
	...
	pui->Create_Cmdbtn(6 , L"BTN_LURKER" , BTN_LURKER);
}

void CHydra::Update_Wireframe(void)
{
	...
	{		
		CUI* pui = NULL;
		pui = new CUI_Wireframe(L"WIRE_HYDRA" , D3DXVECTOR2(interface_pos.x + 165, interface_pos.y + 390 ));
		pui->Initialize();
		CIngame_UIMgr::GetInstance()->add_wireframe_ui(pui);

		CFontMgr::GetInstance()->SetInfomation_font(L"Zerg Hydra" ,interface_pos.x + 320 , interface_pos.y + 390 );
	}
...
void CHydra::SetDamage(const int& idamage , DAMAGE_TYPE edamagetype)
{
