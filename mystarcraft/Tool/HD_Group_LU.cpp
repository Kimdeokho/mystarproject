...
void CHD_Group_LU::MakeTerrain_Group(int istartidx)
{
	...
	for(int i = 0; i < 2; ++i)
	{
		for(int j = 0; j < 2; ++j)
		{
			...
			SetTerrainInfo(TERRAIN_HIGHDIRT , GROUP_LU , m_cursequence , 0);

			if(m_cursequence <= 2)
				SetTerrainInfo(TERRAIN_HIGHDIRT , GROUP_LU , m_cursequence , 1);
...
void CHD_Group_LU::OverlapSequence_RD_2(void)
{
	if(0 == m_cursequence)
	{
		...
		else if(m_MyBrush->GetOverlap_GroupArea(GROUP_L , GROUP_RD))
		{
		...
		if(m_MyBrush->GetOverlap_GroupArea(GROUP_L , GROUP_LD) &&
			...
			m_MyBrush->GetOverlap_GroupArea(GROUP_L , GROUP_LD) &&
			m_MyBrush->GetOverlap_GroupArea(GROUP_R , GROUP_LD))
		{
			SetTerrainInfo(m_terrain_id , GROUP_FLAT , 0 , 0);
			CTileMgr::GetInstance()->SetTerrain(m_oriidx - SQ_TILECNTX , m_sendTerrainInfo , m_bTerrainDelete);
...
void CHD_Group_LU::OverlapSequence_RD_3(void)
{
	...
	else if(1 == m_cursequence)
	{
		...
		if(GROUP_RD == ptemp1->byGroup_ID &&
			GROUP_RU == ptemp2->byGroup_ID && 8 != ptemp3->byGroup_sequence)
		{
			SetTerrainInfo(m_terrain_id , GROUP_FLAT , 0 , 0);
			CTileMgr::GetInstance()->SetTerrain(m_oriidx + SQ_TILECNTX , m_sendTerrainInfo , m_bTerrainDelete);
			CTileMgr::GetInstance()->SetTerrain(m_oriidx + SQ_TILECNTX - 2 , m_sendTerrainInfo , m_bTerrainDelete);
