...
void CHD_Group_R::MakeTerrain_Group(int istartidx)
{
	...
	for(int i = 0; i < 3; ++i)
	{
		for(int j = 0; j < 2; ++j)
		{
			...
			SetTerrainInfo(TERRAIN_HIGHDIRT , GROUP_R , m_cursequence , 0);

			if(m_cursequence <= 1 || m_cursequence == 3)
				SetTerrainInfo(TERRAIN_HIGHDIRT , GROUP_R , m_cursequence , 1);
...
void CHD_Group_R::OverlapSequence_L_0(void)
{
	...
	else if(2 == m_cursequence)
	{
		...
		if(GROUP_LU == ptemp1->byGroup_ID && GROUP_LD == ptemp2->byGroup_ID)
		{
...
void CHD_Group_R::OverlapSequence_L_2(void)
{
	...
	else if(4 == m_cursequence)
	{
		...
		CTileMgr::GetInstance()->SetTerrain(m_oriidx + SQ_TILECNTX, m_sendTerrainInfo , m_bTerrainDelete);
		SetTerrainInfo(m_terrain_id , GROUP_LD , m_cursequence - 2 , 0);
	}
	else if(5 == m_cursequence)
	{
...
void CHD_Group_R::OverlapSequence_LU_0(void)
{
	...
	else if(2 == m_cursequence)
	{
		...
		if(GROUP_LU == ptemp1->byGroup_ID && GROUP_FLAT == ptemp2->byGroup_ID)
		{
			SetTerrainInfo(m_terrain_id , GROUP_FLAT , 0 , 0);
			CTileMgr::GetInstance()->SetTerrain(m_oriidx + 2 , m_sendTerrainInfo , m_bTerrainDelete);
		}
		else if(GROUP_RD != ptemp2->byGroup_ID)
		{
			SetTerrainInfo(m_terrain_id , GROUP_RU , 0 , 1);
			CTileMgr::GetInstance()->SetTerrain(m_oriidx - SQ_TILECNTX , m_sendTerrainInfo , m_bTerrainDelete);
			SetTerrainInfo(m_terrain_id , GROUP_RU , 2 , 0);
...
void CHD_Group_R::OverlapSequence_RU_0(void)
{
	...
	{
		SetTerrainInfo(m_terrain_id , GROUP_R , m_cursequence , 0);
...
void CHD_Group_R::OverlapSequence_RD_4(void)
{
	if(0 == m_cursequence)
	{
		SetTerrainInfo(m_terrain_id , GROUP_R , m_cursequence , 1);
	}
	else if(1 == m_cursequence)
	{
...
void CHD_Group_R::OverlapSequence_R_4(void)
{
	if(0 == m_cursequence)
	{
		SetTerrainInfo(m_terrain_id , GROUP_R , 0 , 1);
	}
	else if(1 == m_cursequence)
	{
...
void CHD_Group_R::OverlapSequence_R_5(void)
{
	...
	{
		SetTerrainInfo(m_terrain_id , GROUP_R , 1 , 1);
	}
	else if(2 == m_cursequence)
	{
...
void CHD_Group_R::OverlapSequence_LD_2(void)
{
	if(0 == m_cursequence)
	{
		...
		TERRAIN_INFO*	ptemp4 = CTileMgr::GetInstance()->GetTerrain_Info(m_oriidx +3);

		A_Search();

		
		if(	m_TerrainGroup[SEARCH_RD] == GROUP_L &&
			m_TerrainGroup[SEARCH_DOWN] == GROUP_L)
		{
			SetTerrainInfo(m_terrain_id , GROUP_FLAT , 0 , 0);
			CTileMgr::GetInstance()->SetTerrain(m_oriidx , m_sendTerrainInfo , m_bTerrainDelete);
			...
			CTileMgr::GetInstance()->SetTerrain(m_oriidx + SQ_TILECNTX + 3, m_sendTerrainInfo , m_bTerrainDelete);

			if(GROUP_LD == ptemp4->byGroup_ID)
			{
				CTileMgr::GetInstance()->SetTerrain(m_oriidx + 2 , m_sendTerrainInfo , m_bTerrainDelete);
				CTileMgr::GetInstance()->SetTerrain(m_oriidx + 3 , m_sendTerrainInfo , m_bTerrainDelete);
			}
			else
			{
				SetTerrainInfo(m_terrain_id , GROUP_RU , 2, 0);
			...
			for(int i = 0; i < 3; ++i)
			{
				for(int j = 0; j < 2; ++j)
				{
					SetTerrainInfo(m_terrain_id , GROUP_LD , i*2+j , 0);
					CTileMgr::GetInstance()->SetTerrain(m_oriidx + j + SQ_TILECNTX*(i+1) , m_sendTerrainInfo , m_bTerrainDelete);
				}
			}
			return;
		}
		//if(	m_TerrainGroup[SEARCH_RD] == GROUP_LU &&
		//	m_TerrainGroup[SEARCH_DOWN] == GROUP_LU)
		//{
		//	SetTerrainInfo(m_terrain_id , GROUP_FLAT, 0 , 0);
		//	CTileMgr::GetInstance()->SetTerrain(m_oriidx - SQ_TILECNTX , m_sendTerrainInfo , m_bTerrainDelete);
		//	CTileMgr::GetInstance()->SetTerrain(m_oriidx - SQ_TILECNTX + 1 , m_sendTerrainInfo , m_bTerrainDelete);
		//	CTileMgr::GetInstance()->SetTerrain(m_oriidx + 2 + SQ_TILECNTX, m_sendTerrainInfo , m_bTerrainDelete);
		//	CTileMgr::GetInstance()->SetTerrain(m_oriidx + 3 + SQ_TILECNTX, m_sendTerrainInfo , m_bTerrainDelete);
		...
		//		{
		//			SetTerrainInfo(m_terrain_id , GROUP_FLAT, 0 , 0);
		//			CTileMgr::GetInstance()->SetTerrain(m_oriidx + j + SQ_TILECNTX*i , m_sendTerrainInfo , m_bTerrainDelete);
		//		}
		//	}
		...
		else if(GROUP_LU == ptemp3->byGroup_ID || GROUP_FLAT == ptemp3->byGroup_ID)
		{
			SetTerrainInfo(m_terrain_id , GROUP_FLAT , 0 , 0);
			CTileMgr::GetInstance()->SetTerrain(m_oriidx - SQ_TILECNTX , m_sendTerrainInfo , m_bTerrainDelete);
			SetTerrainInfo(m_terrain_id , GROUP_RD , 0 , 0);
