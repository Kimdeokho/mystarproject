...
VOID CGameIocp::OnIoRead(VOID *object, DWORD dataLength)
{
	...
	if (ConnectedUser->ReadPacketForIocp(dataLength))
	{
		while (ConnectedUser->GetPacket(Protocol, Packet, PacketLength))
		{
			//////////////////////////////////////////////////////////////////////////
			// 로비관련 패킷
			if (Protocol == PT_LOBY_ENTER)						// PROTOCOL : PT_CHANNEL_ENTER
				onPT_LOBY_ENTER(ConnectedUser, Packet);
			else if (Protocol == PT_CREATE_ROOM)						// PROTOCOL : PT_CHANNEL_ENTER
				onPT_CREATE_ROOM(ConnectedUser, Packet);
			else if(Protocol == PT_ROOM_ENTER)
